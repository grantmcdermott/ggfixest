...
Here follow some examples that draw on the **fixest** introductory vignette,
...
We'll demonstrate using the staggered treatment example from the introductory 
**fixest** vignette.
...
One nice think about the **ggplot2** API is that it makes changing multiplot
figures simple. For example, if you don't like the presentation of "dodged" 
models in a single frame, then it easy to facet them instead using 
...
However, `iplot` complains if we combine a _list_ of (several) `fixest_multi` 
objects.
